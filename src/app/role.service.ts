...
export class RoleService {
  ...
  GetAllx:any=[]
  async GetAllRole() {    this.spinner.show()

 return new Promise<void>((resolve,reject)=>{
  this.http.get("https://localhost:44372/api/Role/getall").subscribe(
    {
      next: (res) => {
...
GetxById: any // {}
async GetRoleById(id: number)//10
{    this.spinner.show()

  return new Promise<void>((resolve, reject) => {
    this.http.get("https://localhost:44372/api/Role/GetById/"+id).subscribe(
      {
        next: (res) => {
...
{    this.spinner.show()
  ...
  return new Promise<void>((resolve,reject)=>{
  this.http.put("https://localhost:44372/api/Role/Update", Role).subscribe(
    {
      next:()=>{
...
async insert(Role : any)
{    this.spinner.show()

  return new Promise<void>((resolve, reject) => {
    this.http.post("https://localhost:44372/api/Role/Create",Role).subscribe(
      {
        next:()=>{
          resolve();  this.spinner.hide();   this.toaster.success("Success")
        },
...
{    this.spinner.show()
  ...
  return new Promise<void>((resolve,reject)=>{
  this.http.delete("https://localhost:44372/api/Role/Delete/"+id).subscribe(
    {
      next:()=>{
        resolve();  this.spinner.hide();   this.toaster.success("Success")
      },
