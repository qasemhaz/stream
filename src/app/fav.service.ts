...
export class FavService {
  ...
  GetAllx:any=[]
  async GetAllFav() {    this.spinner.show()

 return new Promise<void>((resolve,reject)=>{
  this.http.get("https://localhost:44372/api/Fav/getall").subscribe(
    {
      next: (res) => {
...
GetxById: any // {}
async GetFavById(id: number)//10
{    this.spinner.show()

  return new Promise<void>((resolve, reject) => {
    this.http.get("https://localhost:44372/api/Fav/GetById/"+id).subscribe(
      {
        next: (res) => {
...
async Update(Fav : any)
{    this.spinner.show()

  return new Promise<void>((resolve,reject)=>{
  this.http.put("https://localhost:44372/api/Fav/Update", Fav).subscribe(
    {
      next:()=>{
...
{    this.spinner.show()
  ...
  return new Promise<void>((resolve, reject) => {
    this.http.post("https://localhost:44372/api/Fav/Create",Fav).subscribe(
      {
        next:()=>{
          resolve();  this.spinner.hide();   this.toaster.success("Success")
        },
...
{    this.spinner.show()
  ...
  return new Promise<void>((resolve,reject)=>{
  this.http.delete("https://localhost:44372/api/Fav/Delete/"+id).subscribe(
    {
      next:()=>{
        resolve();  this.spinner.hide();   this.toaster.success("Success")
      },
